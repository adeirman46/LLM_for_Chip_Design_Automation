        ...
        if st.button("🚀 Generate Verilog Module", type="primary", use_container_width=True):
            if not st.session_state.new_module_name.strip():
                ...
                with st.spinner("Generating Verilog with LLM..."):
                    param_defs = ",\n".join([f"    parameter {p['name']} = {p['value']}" for p in st.session_state.new_module_params])
                    ...
                    prompt = (
                        ...
                        f"The module must implement the following functionality: {st.session_state.new_module_desc}\n\n"
                        f"It must have the following parameters, if any:\n{param_defs}\n\n" if st.session_state.new_module_params else ""
                        f"It must have the following ports:\n{port_defs}\n\n"
